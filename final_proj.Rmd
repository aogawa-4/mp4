...
ggplot(sum_film, aes(x = budget, y = rating_info, color = as.factor(production_year))) +
  geom_point() 

# budget and gross
ggplot(sum_film, aes(x = budget, y = sum_gross, color = as.factor(production_year))) + geom_point()
```


```{r}
ggplot(sum_film, aes(x = sum_gross, y = rating_info, color = as.factor(production_year))) + geom_point()
```
...
ggplot(data_sum, aes(x = rating_info, fill = oscar))+ 
  geom_bar( position = "stack") +
  labs(y = "Total Number" ,x = "Rating") 
...
#change the genre 
mod <- lm(rating_info ~ genre_info + gross + oscar, data_sum)
